...
parser.add_argument('--accurate', help='''If this parameter is 1, the MC-dropout based early stop mechanism will be activated with two sets of uncertainty cutoff, and the prediction will cost more time.
        sensitive mode (the default mode without early stop, output: 'Results/host_lineage.tsv')
        specific mode (enabling the early stop, output: 'Results/host_lineage_specific.tsv')
        accurate mode (enabling the early stop with more stringent uncertainty cutoff, leading to more accurate prediction but returning taxa in higher levels for some inputs, output: 'Results/host_lineage_accurate.tsv')
        (default: 0)'''
        , type=int, default = 0)
