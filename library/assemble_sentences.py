...
def assemble_sentence(dbPath, mclBlastp_file, mclLength, out_fn, contig2mob, mob_mpfLength, contig2rep):
    ...
    print('The assembled sentence matrix:')
    print(sentence)
...
def preprocessing(fa, database, out_fn, t):
    ...
    os.system(f'diamond blastp --threads {t} --sensitive -d {database}/databasemob.dmnd -q {out_fn}/plasmids.faa -o {out_fn}/resmob.tab -c1 -k 1')
    os.system(f"awk '{{print $1,$2,$11}}' {out_fn}/resmob.tab > {out_fn}/resmob.tab.abc")
    os.system(f'rm {out_fn}/resmob.tab')

    os.system(f'diamond blastp --threads {t} --sensitive -d {database}/databasempf.dmnd -q {out_fn}/plasmids.faa -o {out_fn}/resmpf.tab -c1 -k 1')
    ...
    os.system(f'diamond blastp --threads {t} --sensitive -d {database}/database.dmnd -q {out_fn}/plasmids.faa -o {out_fn}/resp.tab -c1 -k 1')
